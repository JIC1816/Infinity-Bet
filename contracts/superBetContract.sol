...
contract superBetContract is EnetscoresConsumer {
    ...
    function resolveWinner(uint32 _gameId, bytes32 _requestIdGameResolve)
        public
        onlyOwner
    {
        ...
        require(
            keccak256(abi.encodePacked("finished")) ==
                keccak256(abi.encodePacked(matchData.status))
        );
    ...
    function claimRewards(uint32 _gameId) public {
        ...
        require(
            matchs[_gameId].winner ==
                matchs[_gameId].userBets[msg.sender].option,
            "No rewards"
        );
        //Calculo del reward
        uint amount = (matchs[_gameId].userBets[msg.sender].amount *
            matchs[_gameId].userBets[msg.sender].odd) / 10**18; //1 eth
        require(
            amount <= IERC20(tokenAddress).balanceOf(address(this)),
            "Not enought balance in treasury"
        );
        IERC20(tokenAddress).transfer(msg.sender, amount);
    ...
    function calculateReward(uint32 _gameId) public returns (uint) {
        uint amount = (matchs[_gameId].userBets[msg.sender].amount *
            matchs[_gameId].userBets[msg.sender].odd) / 10**18;
